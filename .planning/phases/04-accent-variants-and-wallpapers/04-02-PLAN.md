...
files_modified:
  ...
  - backgrounds/qhd/6-sakura-cherry-blossoms.png
  - backgrounds/qhd/7-ocean-midnight-harbor.png
  - backgrounds/qhd/8-tide-underwater-shrine.png
  - backgrounds/qhd/9-ember-lantern-festival.png
  - backgrounds/qhd/10-moss-forest-shrine.png
autonomous: false
requirements:
  - VAR-05
  - ASST-01
...
must_haves:
  truths:
    ...
    - "Both 4K and QHD wallpapers exist for each variant (4K in backgrounds/, QHD in backgrounds/qhd/)"
  artifacts:
    ...
    - path: "backgrounds/qhd/6-sakura-cherry-blossoms.png"
      provides: "Sakura variant wallpaper (QHD downscale)"
    - path: "backgrounds/qhd/7-ocean-midnight-harbor.png"
      provides: "Ocean variant wallpaper (QHD downscale)"
    - path: "backgrounds/qhd/8-tide-underwater-shrine.png"
      provides: "Tide variant wallpaper (QHD downscale)"
    - path: "backgrounds/qhd/9-ember-lantern-festival.png"
      provides: "Ember variant wallpaper (QHD downscale)"
    - path: "backgrounds/qhd/10-moss-forest-shrine.png"
      provides: "Moss variant wallpaper (QHD downscale)"
  key_links:
...
Output: Executable `clawmarchy-variant` script, 5 new wallpapers in `backgrounds/` (4K), 5 QHD downscales in `backgrounds/qhd/`.
</objective>
...
Human action required: Generate 5 wallpapers using Gemini Pro (Nano Banana interface) with the prompts provided below. Claude cannot generate images -- the user must create these using their Gemini Pro subscription. Save each wallpaper as PNG to the backgrounds/ directory with the specified filename. After all 5 are saved, verify `ls backgrounds/ | wc -l` returns 10.
  ...
  <done>10 4K wallpapers exist in backgrounds/, all are valid PNG files, new wallpapers are dark atmospheric anime scenes paired to variant accent tones. QHD downscales will be created in Task 3.</done>
...
<task type="auto">
  <name>Task 3: Downscale 4K wallpapers to QHD for backgrounds/qhd/</name>
  <files>
    backgrounds/qhd/6-sakura-cherry-blossoms.png
    backgrounds/qhd/7-ocean-midnight-harbor.png
    backgrounds/qhd/8-tide-underwater-shrine.png
  ...
  <action>
After the user confirms wallpapers are saved (Task 2 complete), downscale all 5 new 4K wallpapers to QHD (2560x1440) using ImageMagick. This fulfills the locked decision to ship at both 3840x2160 and 2560x1440 resolutions.

QHD versions go in `backgrounds/qhd/` subdirectory to avoid doubling Omarchy's wallpaper cycler count (the cycler reads from `backgrounds/` only, not subdirectories).

1. Create the `backgrounds/qhd/` directory:
```bash
mkdir -p backgrounds/qhd
```

2. Downscale each wallpaper:
```bash
for img in backgrounds/6-sakura-cherry-blossoms.png \
           backgrounds/7-ocean-midnight-harbor.png \
           backgrounds/8-tide-underwater-shrine.png \
           backgrounds/9-ember-lantern-festival.png \
           backgrounds/10-moss-forest-shrine.png; do
    convert "$img" -resize 2560x1440 -quality 95 "backgrounds/qhd/$(basename "$img")"
done
```

3. Verify each QHD file exists and has correct dimensions using `identify` (ImageMagick):
```bash
identify backgrounds/qhd/*.png
```
Each should report 2560x1440.

NOTE: If ImageMagick `convert` is not available, use `magick` (ImageMagick v7 CLI). The command is: `magick input.png -resize 2560x1440 -quality 95 output.png`.
  </action>
  <verify>
    - `ls backgrounds/qhd/ | wc -l` returns 5
    - `file backgrounds/qhd/6-sakura-cherry-blossoms.png` shows PNG image data
    - `file backgrounds/qhd/7-ocean-midnight-harbor.png` shows PNG image data
    - `file backgrounds/qhd/8-tide-underwater-shrine.png` shows PNG image data
    - `file backgrounds/qhd/9-ember-lantern-festival.png` shows PNG image data
    - `file backgrounds/qhd/10-moss-forest-shrine.png` shows PNG image data
    - `identify backgrounds/qhd/6-sakura-cherry-blossoms.png` reports 2560x1440
    - `identify backgrounds/qhd/10-moss-forest-shrine.png` reports 2560x1440
  </verify>
  <done>
    5 QHD (2560x1440) wallpapers exist in backgrounds/qhd/, downscaled from the 4K originals. Both resolutions are shipped per locked decision.
  </done>
</task>
...
3. `ls backgrounds/*.png | wc -l` returns 10 (5 existing + 5 new 4K wallpapers)
4. `ls backgrounds/qhd/*.png | wc -l` returns 5 (QHD downscales of new wallpapers)
5. All wallpaper files are PNG images (not zero-byte or corrupt)
6. `file backgrounds/6-sakura-cherry-blossoms.png` shows PNG image data
7. `file backgrounds/qhd/6-sakura-cherry-blossoms.png` shows PNG image data
8. `identify backgrounds/qhd/10-moss-forest-shrine.png` reports 2560x1440 dimensions
</verification>

<success_criteria>
- `clawmarchy-variant` script is executable and works for all 6 variants
- 10 wallpapers exist in `backgrounds/` directory (4K resolution)
- 5 QHD wallpapers exist in `backgrounds/qhd/` directory (2560x1440 downscales)
...
- Both 4K and QHD resolutions shipped per locked decision
</success_criteria>
