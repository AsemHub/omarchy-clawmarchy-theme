...
must_haves:
  truths:
    - "vscode.json contains colorCustomizations data with AMOLED black backgrounds and accent indicators, deliverable as a reference for manual user application"
    - "Omarchy's omarchy-theme-set-vscode only reads name and extension -- colorCustomizations are NOT auto-applied; this is a known framework limitation"
    - "A user-facing instruction comment is embedded in vscode.json explaining how to manually apply colorCustomizations to VS Code settings.json"
...
**User instruction (REQUIRED -- framework limitation disclosure):**
Add a top-level `"_comment"` key in the JSON with a brief instruction explaining that `colorCustomizations` must be manually added to VS Code's `settings.json` under `workbench.colorCustomizations`. Example value:
```
...
```
This ensures anyone reading vscode.json understands the manual step required.
...
- Contains `"_comment"` key with instruction about manual application
- Does NOT contain any template syntax
- Total keys in [Tokyo Night] object: at least 25 color overrides
  </verify>
  <done>vscode.json contains name, extension (unchanged), colorCustomizations with 25+ AMOLED black workspace color overrides and accent-colored active indicators scoped to Tokyo Night theme, and a _comment field explaining the manual application step required due to Omarchy framework limitation</done>
</task>
