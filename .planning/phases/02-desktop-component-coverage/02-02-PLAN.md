...
must_haves:
  truths:
    ...
    - "Mako border is full border (all edges), not left-edge stripe -- mako does not support per-side borders; this is the closest native approximation of the locked left-edge stripe decision"
    - "Mako text-color is uniform (#E8E0D0) -- mako does not support separate app-name vs body text coloring; the locked decision for accent-colored app name cannot be achieved natively and is a known limitation"
    - "Hyprlock displays a large accent-colored clock over wallpaper with ~40-50% dimming via semi-transparent black overlay"
...
2. **Global color defaults (after include):**
   - `text-color=#E8E0D0` (foreground -- applies to ALL notification text uniformly)
...
**Known limitations (from locked decisions that mako cannot implement):**
- **App name in accent color:** Locked decision says "App name displayed in accent color, summary and body text in white/gray." Mako only has a single `text-color` directive that applies to ALL text (app name, summary, body). There is no `app-name-color` or per-element color support. All text will be #E8E0D0 (foreground). This is a platform limitation.
- **Left-edge border stripe:** Locked decision says "Border is left-edge stripe, not full border." Mako `border-size` is uniform (all edges). A thin full border (3px) is the closest native approximation.
...
- Does NOT contain `app-name-color` (not a valid mako directive)
  </verify>
  <done>mako.ini exists with core.ini include, AMOLED black background, three urgency levels using accent/yellow/red border colors, and documented limitations for app-name coloring and left-edge-only border (neither supported by mako)</done>
</task>
...
**Wallpaper dimming overlay (~40-50% per user decision):**
Add a second `background {}` block that renders a semi-transparent black overlay on top of the wallpaper. Hyprlock layers multiple background blocks additively. This block uses no `path` (no wallpaper image) and sets `color` to a semi-transparent black:
```
background {
    monitor =
...
```
This achieves ~45% dimming (within the user's 40-50% range). The Omarchy structural config's `background {}` block renders the wallpaper with blur; this theme-added block renders a dark overlay on top of it.
...
- Wallpaper dimmed ~40-50% via semi-transparent black overlay background block (see above)
...
- Contains `background {` block with `color = rgba(0, 0, 0, 0.45)` (dimming overlay)
- The dimming background block does NOT contain a `path` directive (overlay only, no wallpaper image)
...
- Does NOT contain `input-field {` block (that is in Omarchy structural config)
...
- No other content besides optional trailing newline
  ...
  <done>hyprlock.conf defines 5 color variables with AMOLED black and accent values, a semi-transparent black overlay background block for ~45% wallpaper dimming, and a large clock label widget; chromium.theme contains the AMOLED black RGB triplet 0,0,0</done>
</task>
